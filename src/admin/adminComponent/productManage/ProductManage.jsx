...
export const ProductManage = () => {
    ...
    let arrImage = Object.entries(arrImg).map(([k, v]) => [v].toString()); // convert object to array
    let arrTitleImage = Object.entries(tileImg).map(([k, v]) => [v].toString()); // convert object to array
    ...
    let handleEditProduct = (item) => {
        ...
        arrImage = item.arrImage
        arrTitleImage = item.arrTitleImage
    ...
    return (
        <div className="user-manage-container">
                    ...
                    <div className='arr-img-container'>{Array.from(Array(imgLength), (e, i) => {
                        return <div className='content-container' key={i}> <div>Ảnh {i + 1}</div>
                            <div className='content'>
                                <input value={arrTitleImage.filter((item, index) => { if (index === i) return item })} placeholder='tiêu đề ảnh' type="text" onChange={(e) => handleBeginBuilDataImg(i + 'a', e.target.value)} />
                                <input value={arrImage.filter((item, index) => { if (index === i) return item.toString() })} placeholder='đường link ảnh' type="text" onChange={(e) => handleBeginBuilDataImg(i, e.target.value)} />
                                {arrImage.map((item, index) => { if (index === i) return <div key={index} className='content-image' style={{ backgroundImage: `url(${index === i ? item : null})` }}> </div> })}
                            </div>
                        </div>
                    })}</div>
                </div>
            ...
            {arrImage.map((item, index) => { return <div key={index} className='content-image' style={{ backgroundImage: `url(${item})` }}> </div> })}
        </div >
    )
}
